...
<body>
                            ...
                            %>
                                <div class="d-flex tutorial-item mb-4 note-item"
                                    ...
                                    <div>
                                        ...
                                        <p>
                                            <a href="<%=request.getContextPath()%>/viewNote/<%=note.getId()%>"
    ...
    <script>
        document.addEventListener("DOMContentLoaded", () => {
            ...
            const notesContainer = document.querySelector(".col-lg-8");

            // Create a "no results" message and append it to the notes container
            const noResultsMessage = document.createElement("p");
            noResultsMessage.textContent = "No notes match your search criteria.";
            noResultsMessage.style.display = "none"; // Hide initially
            ...
            searchInput.addEventListener("input", () => {
                ...
                notes.forEach(note => {
                    ...
                    if (
                        ...
                        highlightText(note, searchTerm); // Highlight matching terms
                        hasResults = true;
                    } else {
                        note.style.display = "none"; // Hide the note
                        clearHighlight(note); // Clear previous highlights
                    }
                });
            ...
            function highlightText(note, searchTerm) {
                ["data-title", "data-description", "data-subject"].forEach(attr => {
                    const textElement = note.querySelector(`[data-${attr.split('-')[1]}]`);
                    if (textElement) {
                        const originalText = note.getAttribute(attr);
            ...
            function clearHighlight(note) {
                ["data-title", "data-description", "data-subject"].forEach(attr => {
                    const textElement = note.querySelector(`[data-${attr.split('-')[1]}]`);
                    if (textElement) {
                        textElement.innerHTML = note.getAttribute(attr);
                    }
                });
            }
        });
